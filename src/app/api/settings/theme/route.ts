...
const themeSchema = z.object({
  ...
  /** Optional admin terminology preferences used by AI recipe generation. One rule per line: "from => to". */
  foodTerminologyOverrides: z.string().max(4000).optional(),
...
export async function PUT(request: Request) {
  try {
    ...
    const { menuTimezone, themePreset, backgroundImageUrl, managementLanguage, restaurantName, menuCarouselStyle, slotTimes, snowfallEnabled, snowfallStart, snowfallEnd, descriptionTone, foodTerminologyOverrides, restaurantVibeImageKey, restaurantVibeImageUrl, currency, menuTranslationLanguage1, menuTranslationLanguage2, tableOrderingEnabled, ...themeData } = parsed.data
    const newSettings = {
      ...currentSettings,
      theme: { ...(currentSettings.theme as object ?? {}), ...themeData, ...(menuCarouselStyle !== undefined && { menuCarouselStyle }), ...(descriptionTone !== undefined && { descriptionTone }), ...(foodTerminologyOverrides !== undefined && { foodTerminologyOverrides }), ...(restaurantVibeImageKey !== undefined && { restaurantVibeImageKey }), ...(restaurantVibeImageUrl !== undefined && { restaurantVibeImageUrl }) },
