...
import { Check, CheckCircle2, Clock, CreditCard, Crown, Loader2, ShieldCheck, Zap } from 'lucide-react'
...
function RegisterForm() {
  ...
  const annualSavings = Number.isFinite(monthlyPrice) && Number.isFinite(annualPrice)
    ? Math.max(0, monthlyPrice * 12 - annualPrice)
    : 0
  ...
  const comingSoonFeatures = ['POS & order management', 'HR, shifts & payroll']
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              >
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Cancel anytime. No long-term commitment.</p>
                    </div>
                  </div>
                  {plan === 'monthly' ? <CheckCircle2 className="h-5 w-5 text-amber-600" /> : null}
                </div>
                <div className="mt-4 flex flex-wrap items-end gap-2 text-slate-950">
                  {monthlyDiscounted ? (
                    <>
                      <span className="text-3xl font-bold">{pricing.currency} {monthlyDiscounted}</span>
                      <span className="pb-1 text-lg font-semibold text-slate-400 line-through">{pricing.currency} {pricing.priceMonthly}</span>
                    </>
                  ) : (
                    <span className="text-3xl font-bold">{pricing.currency} {pricing.priceMonthly}</span>
                  )}
                ...
                <div className="mt-4 border-t border-slate-200 pt-3">
                  <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Coming soon</p>
              ...
              >
                {annualSavings > 0 ? (
                  <div className="absolute -top-3 right-4 rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold text-white shadow">
                    Save {pricing.currency} {annualSavings}
                  </div>
                ) : null}
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-slate-950">Annual</p>
                      <p className="text-sm text-slate-500">
                        Best value. Save compared to monthly billing.
                      </p>
                    </div>
                  </div>
                  {plan === 'annual' ? <CheckCircle2 className="h-5 w-5 text-amber-600" /> : null}
                </div>
                <div className="mt-4 flex flex-wrap items-end gap-2 text-slate-950">
                  {annualDiscounted ? (
                    <>
                      <span className="text-3xl font-bold">{pricing.currency} {annualDiscounted}</span>
                      <span className="pb-1 text-lg font-semibold text-slate-400 line-through">{pricing.currency} {pricing.priceAnnual}</span>
                    </>
                  ) : (
                    <span className="text-3xl font-bold">{pricing.currency} {pricing.priceAnnual}</span>
                  )}
